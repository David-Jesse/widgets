...
import SwipeContainer from "./swipe-container.tsx";
...
const TimelineBar = ({centeredMonthIndex, data, onSwipeLeft, onSwipeRight}: TimelineBarProps) => {
    return (
        <div className={'absolute bottom-6 flex w-[280px] items-start'}>
            <SwipeContainer onSwipeLeft={onSwipeLeft} onSwipeRight={onSwipeRight}>
                <motion.div
                    ...
                    animate={{
                        transform: `translate(${-(centeredMonthIndex * 235) + 128}px)`
                    }}
                    transition={{
                        duration: 0.6,
                        type: 'spring',
                ...
                >
                    {data.map((monthData, dataIndex) => (
                        <div className={'flex items-end'} key={monthData.month}>
                            ...
                            >
                                <div
                                    className={`flex w-[11px] flex-col items-center justify-center rounded-full ${
